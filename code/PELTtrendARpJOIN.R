...
fit.trendARpJOIN=function(data,cpts,p,dates=NULL,plot=T,fit=T,add.ar=F,title="Data",pred=F){
  ...
  trendARpsegfit=function(data,start,previousbeta,p,pred=F){
    ...
    if(pred==F){
      ...
      return(list(coef=c(coef(trendfit),coef(arfit),arfit$sigma2),arfit=fitted(arfit),
                  trendfit=fitted(trendfit)+previousbeta-previousbeta*(t-start)/n))
      # Note that the beta here needs to be divided by n for interpretability as a slope parameter but we
      # need it without the division for the fitting as we don't pass the previous n across with the beta.
    }
    else{
  ...
  FIRSTtrendARpsegfit=function(data,start,p){
    ...
    ## divide by n on the coef for beta because of the transformation to X
    #print(coef(trendfit)[2])
    return(list(coef=c(coef(trendfit)[2]+coef(trendfit)[1],coef(arfit),arfit$sigma2),arfit=fitted(arfit),trendfit=fitted(trendfit)))
    # sum both the trend and the intercept as this is the beta needed to go into the calculation for the next segment.
    # this is NOT the beta for the segment!!!!
  }
  
  
  cpts=c(0,cpts,length(data))
  segments=list()
  coeffs=matrix(NA,nrow=length(cpts)-1,ncol=p+2)# + 2 as AR1 and sigma2
